...
export default function ContactContent() {
  const [email, setEmail] = useState(defaultSiteContent.contactEmail);
  const [instagram, setInstagram] = useState(defaultSiteContent.contactInstagram);
  const [whatsapp, setWhatsapp] = useState(defaultSiteContent.contactWhatsapp);

  useEffect(() => {
    const unsub = onSnapshot(doc(db, "siteContent", "main"), (snapshot) => {
      const data = snapshot.data();
      if (!data) return;
      setEmail(data.contactEmail || defaultSiteContent.contactEmail);
      setInstagram(data.contactInstagram || defaultSiteContent.contactInstagram);
      setWhatsapp(data.contactWhatsapp || defaultSiteContent.contactWhatsapp);
    });
  ...
  const defaultMessage = useMemo(
    () => encodeURIComponent("Hola Elussila, me interesa una obra de tu galería."),
    []
  );
      ...
      <section className="rounded-3xl border border-zinc-800 bg-zinc-900/40 p-8 md:p-12">
        ...
        <div className="grid gap-6 text-zinc-300 md:grid-cols-2">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6">
            ...
            <a
              href={`https://wa.me/${whatsapp}?text=${defaultMessage}`}
              target="_blank"
              rel="noreferrer"
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6">
            ...
            <a href={`mailto:${email}`} className="text-lg text-white underline underline-offset-4">
              {email}
            </a>
          </div>
